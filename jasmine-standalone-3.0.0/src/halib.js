...
// Basic functions
 const cp = (...fns) => arg => _cp(fns, arg);
 const chain = (...fns) => arg => _chain(fns, arg);

 const append = xs => ys => _append(xs, ys);
 const head = xs => xs[0];
 const last = xs => xs[_len(xs) - 1];
 const init = xs => _copyA(xs, 0, _nul(xs) ? 0 : _len(xs) - 1, isString(xs));
 const tail = xs => _slice(xs, _nul(xs) ? 0 : 1, _len(xs), isString(xs));
 const nul = _nul;
 const len = _len;
 const copy = xs => _copyA(xs,0,_len(xs));
 const cons = x => xs => _cons(x, xs);
 const consR = xs => x => _consR(xs, x);
 const uncons = xs => [head(xs), tail(xs)];
 const nth = ind => xs => xs[ind + (ind < 0 ? _len(xs) : 0)];
 const each = fn => xs => _each(fn, xs);
 const id = _id;
 const flip = fn => x => y => fn (y) (x);
 const not = x => !x;
 const iter = npred => fn => x => _iter (npred, fn, x);
 const until = pred => fn => x => _until(pred, fn, x);
 const equal = (x, y) => _equal(x, y);
 const eq = x => y => _equal(x, y);
 const equalBy = _eq;


// List transformations
 const map = fn => xs => _map(fn, xs);

 const concat = xss => _concat(xss, _id, _foldl((z, x) => z + _len(x), 0, xss));
 const concatMap = fn => xs => _concat(xs, fn, 0);

 const reverse = _reverse;
 const rotate = n => xs => _rotate(n, xs);
 const intersperse = sep => xs => _intersperse(sep, xs);

 const group = xs => _groupBy(_id, xs, false, fn_push);
 const count = xs => _group(xs, true, fn_one, fn_count, _id);
 const section = xs => _section(xs, _equal, secFn);
 const sectionH = xs => _section(xs, _equal, secHFn, true);
 const sectionBy = fn => xs => _section(xs, _len(fn) > 1 ? fn : _eq(fn), secFn);
 const sectionHBy = fn => xs => _section(xs, _len(fn) > 1 ? fn : _eq(fn), secHFn, true);
 const splitBy = n => xs => _splitBy(n, xs);

 const shuffle = _shuffle;
 const flatten = _flatten;
 const prod = xs => ys => _prod(pair, true, xs, ys);

 const diags1 = xss => _diagonals(xss, true);
 const diags2 = xss => _diagonals(xss, false);


// Combinatorics
 const subsequences = _subsequences;
 const cartProd = _cartProd;
 const combinations = k => xs => _combinations(k, xs);
 const combRep = k => xs => _combRep(k, xs);
 const permutations = xs => _permutations(xs);
 const permK = k => xs => _permutations(xs, k);
 const permRep = k => xs => cartProd (_repeat (k, xs));

 const bindL = xs => fn => _concat (xs, fn, 0);
 const returnL = _returnL;
 const guardL = b => fn => b ? fn(0) : [];
 const liftM2L = fn => xs => ys => _prod(uncurry(fn), false, xs, ys);
 const foldM = fn => z => xs => _foldr ((x, res) => a => bindL(fn(a, x))(res), _returnL, xs) (z);

 const bindR = fn1 => fn2 => x => fn2 (fn1 (x)) (x);
 const returnR = x => () => x;
 const liftM2R = fn => fn1 => fn2 => x => uncurry(fn) (fn1(x), fn2(x));

 const divx = fn1 => fn2 => x => [fn1 (x), fn2 (x)];


// Math
 const add = (x, y) => y === undefined ? y => x + y : x + y;
 const sub = (x, y) => y === undefined ? y => x - y : x - y;
 const mul = (x, y) => y === undefined ? y => x * y : x * y;
 const div = (x, y) => y === undefined ? y => x / y : x / y;
 const fact = _fact;
 const negate = x => -x;
 ...
 const min = (x, y) => y < x ? y : x;

 const random = (min, max) => min <= max ? min + Math.floor((max - min + 1) * Math.random()) : random(max, min);
 const choice = xs => xs[random(0, _len(xs) - 1)];


// Objects
 const copyO = obj => Object.assign({}, obj);
 const clone = obj => _clone(obj);

 const pick = (...keys) => obj => _pick(keys, obj);
 const pickBy = pred => obj => _pickBy(pred, obj);
 const mapO = fn => obj => _mapObj(fn, obj);
 const select = (...keys) => objs => map (pick(...keys)) (objs);
 const invert = obj => _invert(obj, _id, (_,key) => key);
 const invertAll = obj => _invert(obj, returnL, grOAdd);
 const extend = obj1 => obj2 => Object.assign({}, obj1, obj2);
 const omit = (...keys) => obj => _omit(keys, obj);


// Reducing lists
 const foldl = fn => z => xs => _foldl(fn, z, xs);
 const foldr = fn => z => xs => _foldr(fn, z, xs);
 const foldl1 = fn => xs => _foldl(fn, NUL, xs);
 const foldr1 = fn => xs => _foldr(fn, NUL, xs);

 const any = pred => xs => _any(pred, xs);
 const all = pred => xs => _all(pred, xs);

 const none = pred => xs => !_any(pred, xs);
 const or = xs => _any(_id, xs);
 const and = xs => _all(_id, xs);

 const sum = _sum;
 const product = xs => _foldl (mul, 1, xs);
 const mean = xs => sum(xs) / _len(xs);
 const median = xs => _median(xs, gt, lt, _id);
 const sumBy = fn => xs => _foldl ((z, x) => z + fn(x), 0, xs);
 const meanBy = fn => xs => sumBy (fn) (xs) / _len(xs);
 const medianBy = fn => xs => _median(xs, _gt(fn), _lt(fn), fn);

 const maximum = xs => best (xs, gt);
 const minimum = xs => best (xs, lt);

 const maxLen = xss => _len(xss) ? _len(best (xss, _gt(_len))) : undefined;
 const minLen = xss => _len(xss) ? _len(best (xss, _lt(_len))) : undefined;

 const uniq = xs => _isUniq(xs, _id);

 const countElem = e => xs => _countE(e, _equal, xs);
 const countSeq = xs => ys => _seqAll(xs, ys, 0, _inc, NUL);
 const toObj = xss => _foldl ((z, p) => (z[p[0]] = p[1], z), {}, xss);


// Building lists
 const scanl = fn => z => xs => _scanl(fn, z, xs);
 const scanr = fn => z => xs => _scanr(fn, z, xs);
 const scanl1 = fn => xs => _scanl(fn, NUL, xs);
 const scanr1 = fn => xs => _scanr(fn, NUL, xs);

 const iterate = npred => fn => x => _iterate (npred, fn, x);
 const untilA = pred => fn => x => _untilA(pred, fn, x);
 const times = n => fn => _times(n, fn);
 const repeat = (n, x) => isString(x) ? (n<0?'':x.repeat(n)) : _repeat(n,x,prim(x)?_id:isArr(x)?copy:copyO);
 const replicate = (n, x) => _repeat(n, x, prim(x) ? _id : clone);
 const cycle = (n, xs) => _cycle(n, xs);

 const from = (first, n, step = 1) => {
    if (step === 0) step = 1;
 ...
 const values = Object.values || (obj => _map (key => obj[key], keys(obj)));
 const entries = Object.entries || (obj => _map (key => [key, obj[key]], keys(obj)));


// Sublists
 const take = n => xs => n <= 0 || n > _len(xs) ? outT(xs, n) : _copyA(xs, 0, n, isString(xs));
 const drop = n => xs => n <= 0 || n > _len(xs) ? outD(xs, n) : _slice(xs, n, _len(xs), isString(xs));
 const takeLast = n => xs => drop (_len(xs) - n) (xs);
 const dropLast = n => xs => take (_len(xs) - n) (xs);
 const takeFrom = ind => n => xs => n <= 0 ? emptyAS(xs) : _slice(xs,ind,trans(ind,_len(xs))+n,isString(xs));
 const splitAt = n => xs => [take(n) (xs), drop(n) (xs)];

 const takeWhile = pred => xs => _while(take, pred, xs);
 const dropWhile = pred => xs => _while(drop, pred, xs);
 const span = pred => xs => _while(splitAt, pred, xs);

 const takeLastWhile = pred => xs => _whileLast(drop, pred, xs);
 const dropLastWhile = pred => xs => _whileLast(take, pred, xs);
 const spanLast = pred => xs => _whileLast(splitAt, pred, xs);

 const nths = (n, start = 0) => xs => _nths(n, start, xs);
 const tails = xs => _inits (xs, 0, _len(xs), 1, tail);
 const inits = xs => _inits (xs, _len(xs), _len(xs), -1, init);


// Searching
 const elem = e => xs => _elemBy(e, xs, _equal);
 const notElem = e => xs => !_elemBy(e, xs, _equal);
 const elemSorted = e => xs => _elemIndexSorted(e, xs) !== -1;

 const startsWith = xs => ys => prefix (xs, ys, _len(xs), 0);
 const endsWith = xs => ys => prefix (xs, ys, _len(xs), _len(ys) - _len(xs));
 const seq = xs => ys => _seq (xs, ys) !== -1;

 const find = pred => xs => _find (pred, xs);
 const filter = pred => xs => _filter(pred, xs);
 const reject = pred => xs => _reject(pred, xs);
 const findLast = pred => xs => _findLast (pred, xs);
 const partition = pred => xs => _partition(pred, xs);


// Indexing
 const elemIndex = e => xs => _index(xs, e);
 const elemIndices = e => xs => _elemIndices(fn_eq, _equal, [], xs, e);
 const elemIndexLast = e => xs => _indexLast(_equal, xs, e);
 const findIndex = pred => xs => _find (pred, xs, true);
 const findIndices = pred => xs => _elemIndices(fn_each, pred, [], xs);
 const findIndexLast = pred => xs => _findLast (pred, xs, true);

 const elemIndexSorted = e => xs => _elemIndexSorted(e, xs);

 const seqIndex = xs => ys => _seq (xs, ys);
 const seqIndices = xs => ys => _seqAll (xs, ys, [], _indices, NUL);


// Zipping
 const zip = xs => ys => _zip (pair, true, xs, ys);
 const zipWith = fn => xs => ys => _zip (uncurry(fn), false, xs, ys);
 const unzip = xss => _len(xss)===2?_zip(pair,true,xss[0],xss[1]):_unzip(xss,_len(xss)===3?_zip3:_zipM,minLen);
 const transpose = unzip;
 const tpLongest = xss => _unzip(xss, _zipLongest, maxLen);
 const rotateL = xss => _unzip(xss, _rotLeft, minLen);
 const rotateR = xss => _unzip(xss, _rotRight, minLen);
...
// Special lists
 ...
 const unchars = _unchars;
 const join = sep => arr => arr.join(sep);
 const chr = ord => String.fromCharCode(ord);

 const upper = c => isChar(c) && c[0] !== toLower(c[0]);
 const lower = c => isChar(c) && c[0] !== toUpper(c[0]);
 const alpha = c => isChar(c) && toUpper(c[0]) !== toLower(c[0]);
 const digit = c => isChar(c) && '0' <= c[0]  && c[0] <= '9';
 const space = c => isChar(c) && (c = ord(c[0]), c === 32 || (c > 8 && c < 14));
 const alnum = c => alpha(c) || digit(c);


// Deleting
 const del = e => xs => bothStr(e, xs) ? xs.replace(e,'') : secc(_index(xs, e), 1, xs, false, NUL);
 const delAll = e => xs => _delAll(e, xs, NUL);
 const deleteSeq = seq => xs => secc(_seq(seq, xs), _len(seq), xs, true, NUL);
 const deleteSeqAll = seq => xs => _seqAll (seq, xs, [], _delall);
 const deleteAt = ind => xs => _sec(ind, 1, xs, false, NUL);
 const deleteFrom = ind => n => xs => _sec(ind, n, xs, true, NUL);

 const deleteIndices = indices => xs => _deleteIndices(indices, xs);
 const keepIndices = indices => xs => _deleteIndices(indices, xs, true);

 const replace = e => toIns => xs => bothStr(e,xs) ? xs.replace(e,toIns) : secc(_index(xs,e),1,xs,false,toIns);
 const replaceAll = e => toIns => xs => _delAll(e, xs, toIns);
 const replaceSeq = seq => toIns => xs => secc(_seq(seq, xs), _len(seq), xs, true, toIns);
 const replaceSeqAll = seq => toIns => xs => _seqAll(seq, xs, [], _delall, toIns);
 const replaceAt = ind => toIns => xs => _sec(ind, 1, xs, false, toIns);
 const replaceFrom = ind => n => toIns => xs => _sec(ind, n, xs, true, toIns);

 const insertAt = ind => e => xs => _sec(ind, 0, xs, false, e, gt);
 const insertSeqAt = ind => seq => xs => _sec(ind, 0, xs, true, seq, gt);


// Set operations
 const nub = xs => ops(nubSet, nubSet, null, xs, _id);
 const minus = xs => ys => ops(writeMap, readMap, xs, ys, _id);
 const deleteAll = xs => ys => ops(newSet, _dall, ys, xs, _id);
 const keepAll = xs => ys => ops(newSet, _keep, ys, xs, _id);
 const intersect = xs => ys => ops(newSet, intSet, xs, ys, _id);
 const difference = xs => ys => ops(newSet, nubSet, xs, ys, _id);
 const union = xs => ys => ops(nubSet, nubSet, ys, xs, _id);
 ...
 const minusSorted = xs => ys => mdk(xs, ys, _inc, true);
 const deleteAllSorted = xs => ys => mdk(ys, xs, _id, true);
 const keepAllSorted = xs => ys => mdk(ys, xs, _id, false);
 const intersectSorted = xs => ys => isd(xs, ys, _inc, false);
 const differenceSorted = xs => ys => isd(xs, ys, _id, true);
 const unionSorted = xs => ys => _union(xs, ys);


// Sorting
 const asc = fn => (x, y) => _cmp (fn(x), fn(y));
 const desc = fn => (x, y) => _desc (fn(x), fn(y));
 ...
 const sort = xs => qsort(xs, gt);
 const sortDesc = xs => qsort(xs, lt);

 const sortBy = fn => xs => msort(xs, gtfn(fn));
 const sortDescBy = fn => xs => msort(xs, ltfn(fn));

 const sortN = n => xs => _sortN(n, xs, sort, gt);
 const sortNDesc = n => xs => _sortN(n, xs, sortDesc, lt);
 const sortNBy = n => fn => xs => _sortN(n, xs, sortBy(fn), gtfn(fn));
 const sortNDescBy = n => fn => xs => _sortN(n, xs, sortDescBy(fn), ltfn(fn));

 const merge = xs => ys => _mergeSorted(xs, ys);
 const insert = e => xs => _insertSorted(e, xs);

 const mergeBy = fn => xs => ys => _mergeSortedBy(fn, xs, ys);
 const insertBy = fn => e => xs => _insertSortedBy(fn, e, xs);

 const maxBy = fn => (x, y) => gtfn(fn) (y, x) ? y : x;
 const minBy = fn => (x, y) => ltfn(fn) (y, x) ? y : x;
 const maximumBy = fn => xs => best (xs, gtfn(fn));
 const minimumBy = fn => xs => best (xs, ltfn(fn));

 const ordered = xs => _ordered(xs, gt);
 const orderedBy = fn => xs => _ordered(xs, gtfn(fn));


// The "By" operations
 const nubBy = fn => xs => _len(fn) > 1 ? _nubBy(xs, fn) : ops(nubSet, nubSet, null, xs, fn);
 const minusBy = fn => xs => ys => _len(fn) > 1 ? _minusBy(xs,ys,fn) : ops(writeMap,readMap,xs,ys,fn);
 const deleteAllBy = fn => xs => ys => _len(fn) > 1 ? _dkdi(ys,xs,dallBy,fn) : ops(newSet,_dall,ys,xs,fn);
 const keepAllBy = fn => xs => ys => _len(fn) > 1 ? _dkdi(ys,xs,kallBy,fn) : ops(newSet,_keep,ys,xs,fn);
 const intersectBy = fn => xs => ys => _len(fn) > 1 ? _dkdi(xs,ys,intsBy,fn) : ops(newSet,intSet,xs,ys,fn);
 const differenceBy = fn => xs => ys => _len(fn) > 1 ? _dkdi(xs,ys,diffBy,fn) : ops(newSet,nubSet,xs,ys,fn);
 const unionBy = fn => xs => ys => _len(fn) > 1 ? _unionBy(xs, ys, fn) : ops(nubSet, nubSet, ys, xs, fn);

 const uniqBy = fn => xs => _len(fn) > 1 ? _uniqBy(xs, fn) : _isUniq(xs, fn);
 const elemBy = fn => e => xs => _elemBy(e, xs, _len(fn) > 1 ? fn : _eq(fn));

 const groupBy = fn => xs => _len(fn) > 1 ? gcBy(xs, groupAdd, groupInc, fn, true):_groupBy(fn,xs,false,fn_push);
 const countBy = fn => xs => _len(fn) > 1 ? gcBy(xs, countAdd, countInc, fn): _group(xs,true,fn_one,fn_count,fn);

 const grouppBy = fn => xs => _groupBy(fn, xs, true, fn_push_K);

 const groupOBy = fn => xs => _groupObj(xs, returnL, grOAdd, fn, true);
 const countOBy = fn => xs => _groupObj(xs, fn_one, inc, fn);